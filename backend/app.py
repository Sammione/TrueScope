...
class ESGIndex:
    ...
    def _embed_texts(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            # OpenAI batch size limit (check docs, usually safe with reasonably defined chunks)
    ...
    def search(
        ...
        # indices[0] is the list of nearest neighbor indices
...
class EvidenceIndex:
    ...
    def search(self, query: str, top_k: int = 6) -> List[Dict[str, Any]]:
        ...
        # Safe call
        try:
             q_emb = client.embeddings.create(input=[query.replace("\n", " ")], model=EMBED_MODEL).data[0].embedding
...
@app.get("/")
def root():
    return {"message": "TrueScope API running (OpenAI)", "docs": "/docs"}
